...
let TopRatedMovies = useSelector((state) => state.MoviesInCombineReducer.movies);
        ...
        {TopRatedMovies.map((item)=>{
            return(
                <div key={item.id} className="card col-2 m-4" >
                   <Link to={`/movie/top_rated/${item.id}`}> <img src={`https://image.tmdb.org/t/p/w500/${item.poster_path}`} className="card-img-top" alt={item.original_title}/></Link>
