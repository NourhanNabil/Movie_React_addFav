...
let OnTheAirTvShows = useSelector((state) => state.TVshowsInCombineReducer.TVshows);
        ...
        {OnTheAirTvShows.map((item)=>{
            return(
                 <div key={item.id} className="card col-2 m-4" >
                    <Link to={`/tv/on_the_air/${item.id}`}><img src={`https://image.tmdb.org/t/p/w500/${item.poster_path}`} className="card-img-top" alt={item.original_title}/></Link>
